...
/// Mutable version of Persistent op trait
pub trait PersistentOpMut<C: PersistentClient> {
    /// Persistent op의 input type
    type Input;
    ...
    ///     * safe하기만 하면 되므로 상관 없음. functional correctness는 보장하지 않음.
    /// - 같은 client에 대해 언제나 같은 Output을 반환 (idempotent)
    fn persistent_op_mut(&mut self, client: &mut C, input: Self::Input) -> Self::Output;
}
