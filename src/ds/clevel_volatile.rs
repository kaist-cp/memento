...
fn hashes<T: Hash>(t: &T) -> [u32; 2] {
    ...
    [left as u32, if left != right { right as u32 } else { right as u32 + 1 }]
}
...
impl<K: Debug + Display + PartialEq + Hash, V: Debug> Context<K, V> {
    ...
    fn find<'g>(&'g self, key: &K, key_hashes: [u32; 2], guard: &'g Guard) -> Result<Option<FindResult<'g, K, V>>, ()> {
        ...
        for find_result in found.into_iter().rev() {
            if find_result.slot_ptr.tag() == 1 {
                ...
                if last.slot_ptr == slot_ptr || owned_found.iter().any(|x| x.slot_ptr == slot_ptr) {
                    ...
                    find_result.slot.store(Shared::null().with_tag(1), Ordering::Release);
                } else {
...
impl<K: PartialEq + Hash, V> ClevelInner<K, V> {
    fn add_level<'g>(
        ...
        let next_level = if !next_level.is_null() {
            ...
            let next_level = if !next_level.is_null() {
                ...
                first_level
                    .next
                    .compare_exchange(Shared::null(), next_node, Ordering::AcqRel, Ordering::Acquire, guard)
        ...
        loop {
            context = ok_or!(
                self.context
                    .compare_exchange(context, context_new, Ordering::AcqRel, Ordering::Acquire, guard),
                e,
                {
                    context = e.current;
                    context_new = e.new;
                    ...
                    if unsafe { context_ref.first_level.load(Ordering::Acquire, guard).deref() }
                        .data
                        .len()
                        >= next_level_size
                    {
                    ...
                    context_new
                        .last_level
                        .store(context_ref.last_level.load(Ordering::Acquire, guard), Ordering::Relaxed);
                    continue;
                }
            );
    ...
    pub fn resize(&self, guard: &Guard) {
        ...
        loop {
            ...
            println!("[resize] last_level_size: {last_level_size}, first_level_size: {first_level_size}");
                    ...
                    loop {
                        ...
                        for i in 0..SLOTS_IN_BUCKET {
                            for key_hash in key_hashes.clone() {
                                let slot =
                                    unsafe { first_level_ref.data[key_hash].assume_init_ref().slots.get_unchecked(i) };

                                if let Some(slot) = unsafe { slot.load(Ordering::Acquire, guard).as_ref() } {
                        ...
                        println!("[resize] resizing again for ({}, {}, {})...", last_level_size, bid, sid);
            ...
            loop {
                context = ok_or!(
                    ...
                    e,
                    {
                        ...
                        context_new.resize_size = cmp::max(context_new.resize_size, context_ref.resize_size);
                        continue;
                    }
                );
        ...
        for array in arrays.into_iter().rev() {
            ...
            for i in 0..SLOTS_IN_BUCKET {
                for key_hash in key_hashes.clone() {
                    ...
                    if let Ok(slot_ptr) =
                        slot.compare_exchange(Shared::null(), slot_new, Ordering::AcqRel, Ordering::Relaxed, guard)
                    {
...
impl<K: Debug + Display + PartialEq + Hash, V: Debug> Clevel<K, V> {
    ...
    pub fn get_capacity<'g>(&'g self, guard: &'g Guard) -> usize {
        ...
        (unsafe { first_level.deref().data.len() * 2 - last_level.deref().data.len() }) * SLOTS_IN_BUCKET
    }
    ...
    fn insert_inner<'g>(
        ...
        loop {
            if let Ok(result) = self.inner.insert_inner(tid, context, slot, key_hashes, guard) {
    ...
    fn move_if_resized<'g>(
        ...
        loop {
            ...
            insert_result.slot.store(Shared::null().with_tag(1), Ordering::Release);
            ...
            for tid in 0..THREADS {
                ...
                let _ = s.spawn(move |_| {
                    ...
                    for i in 0..RANGE {
                        ...
                        // println!("[test] tid = {tid}, i = {i}, search");
                        if kv.search(&i, &guard) != Some(&i) && kv.search(&i, &guard) != Some(&(i + RANGE)) {
                            panic!("[test] tid = {tid} fail on {i}");
                        }
                    }
                });
