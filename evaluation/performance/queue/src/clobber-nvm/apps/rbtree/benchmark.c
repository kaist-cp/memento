...
void *worker(void *arg) {
    ...
    //printf("inside worker function, before load first half\n");
    //printf("list addr = %p \n", ctx->list);
    ...
    if (ctx->phase == 'L') {
	    ...
	    //printf("key = %s, value = %s \n", ops->key, value);
            doInsert(ctx->tree, ops->key, strlen(ops->key), value, strlen(value));
            TraceOp *t = ops;
            ops = ops->next;
            free(t);
    ...
    while (ops != NULL) {
        switch(ops->opCode) {
            case Insert:
                ...
                break;
            case Update:
                //doUpdate(ctx->tree, ops->key, strlen(ops->key), value, strlen(value));
                break;
            case Read:
...
uint64_t run(char workload, char phase, RbtreePtrType tree, char *tracePath,
    ...
    for (int i = 0; i < threadCount; i++) {
        ...
        RbtreePtrAssign(context->tree, tree);
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, ":f:t:d:w:rh")) != -1) {
        switch(opt) {
            ...
            case 'h':
            default:
                fprintf(stdout, "Benchmark tool for the rbtree data structure.\n");
