...
static pthread_rwlock_t slabs_lock;
...
item *mt_item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes) {
    item *it;
    ptr_para_record(key, strlen(key));
    ptr_para_record(&nkey, sizeof(size_t));
    ptr_para_record(&flags, sizeof(int));
    ptr_para_record(&nbytes, sizeof(int));
    ptr_para_record(&exptime, sizeof(rel_time_t));
    add_func_index(1);
    //pthread_mutex_lock(&cache_lock);
    pthread_rwlock_wrlock(&cache_lock);
    tx_lock();
...
item *mt_item_get_notedeleted(const char *key, const size_t nkey, bool *delete_locked) {
    ...
    ptr_para_record(key, strlen(key));
    ptr_para_record(&nkey, sizeof(size_t));
    //ptr_para_record(delete_locked, sizeof(bool));
...
int mt_item_replace(item *old, item *new) {
    ...
    tx_lock();
    add_func_index(5);
    nvm_ptr_record(old, 8);
    nvm_ptr_record(new, 8);
...
char *mt_defer_delete(item *item, time_t exptime) {
    ...
    add_func_index(8);
    nvm_ptr_record(item, 8);
    ptr_para_record(&exptime, sizeof(time_t));
...
char *mt_add_delta(item *item, int incr, const int64_t delta, char *buf) {
    ...
    add_func_index(9);
    nvm_ptr_record(item, 8);
    ptr_para_record(&incr, sizeof(int));
    ptr_para_record(&delta, sizeof(const int64_t));
    ptr_para_record(buf, strlen(buf));
...
int mt_store_item(item *item, int comm) {
    ...
    add_func_index(10);
    nvm_ptr_record(item, 8);
    ptr_para_record(&comm, sizeof(int));
...
char *mt_item_cachedump(unsigned int slabs_clsid, unsigned int limit, unsigned int *bytes) {
    ...
    tx_lock();
    add_func_index(12);
    ptr_para_record(&slabs_clsid, sizeof(unsigned int));
    ptr_para_record(&limit, sizeof(unsigned int));
    ptr_para_record(bytes, sizeof(unsigned int));
...
char *mt_item_stats(int *bytes) {
    ...
    tx_lock();
    add_func_index(13);
    ptr_para_record(bytes, sizeof(int));
...
void *mt_slabs_alloc(size_t size, unsigned int id) {
    void *ret;
    pthread_rwlock_wrlock(&slabs_lock);
    //pthread_mutex_lock(&slabs_lock);
    tx_lock();
    add_func_index(15);
    ptr_para_record(&size, sizeof(size_t));
    ptr_para_record(&id, sizeof(unsigned int));
    ret = do_slabs_alloc(size, id);
    tx_unlock();
    pthread_rwlock_unlock(&slabs_lock);
...
char *mt_slabs_stats(int *buflen) {
    ...
    tx_lock();
    add_func_index(17);
    ptr_para_record(buflen, sizeof(int));
...
int mt_slabs_reassign(unsigned char srcid, unsigned char dstid) {
    ...
    tx_lock();
    add_func_index(18);
    ptr_para_record(srcid, sizeof(unsigned char));
    ptr_para_record(dstid, sizeof(unsigned char));
...
void thread_init(int nthreads, struct event_base *main_base) {
    ...
    pthread_rwlock_init(&slabs_lock, NULL);
