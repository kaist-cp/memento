...
namespace PiBench
{
  ...
  void benchmark_t::load() noexcept
  {
    ...
    std::cout << "skip_load: " << opt_.skip_load << std::endl;
    std::cout << "if `skip_load` is false, " << opt_.num_ops << " values will be inserted in the load phase" << std::endl;
    ...
    auto elapsed_gen = sw.elapsed<std::chrono::milliseconds>();
    sw.start();
...
#pragma omp for schedule(static)
    ...
    std::cout << "\nOverview:"
              << "\n"
              << "\tGenerate time: " << elapsed_gen << " milliseconds" << std::endl
