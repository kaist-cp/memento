...
// ekosmas: This version of p_utils replaced static memory (allocated in DRAM)
//			with dynamic memory allocation (so that it is alocated in NVMM)
...
namespace utils {
	...
	template <class ET>
	inline bool CAS8(ET volatile *ptr, ET oldv, ET newv) { 
	  bool ret;
	  if ( *((long*) ptr) != (long) oldv) {
	  	return false;
	  }
	  ret = __sync_bool_compare_and_swap_8((long*) ptr, *((long*) &oldv), *((long*) &newv));
	  WFLUSH(BARRIER(ptr););
...
static closure *closures;
static closure *closures2;


void init_closures () 
{
	closures = (closure *) malloc (MAX_THREADS*sizeof(closure));
	closures2 = (closure *) malloc (MAX_THREADS*sizeof(closure));
}
...
void capsule_boundary_opt(int my_id) {
    ...
    MANUAL(PWB_LOW(&closures[my_id]));	
    WFLUSH(PWB(&closures[my_id]));	
}
...
void capsule_boundary_opt(int my_id, void* a, void* b, void* c) {
	...
	MANUAL(PWB_LOW(&closures2[my_id]));											
	MANUAL(PFENCE();)
	WFLUSH(BARRIER(&closures2[my_id]));											
	closures[my_id].data[0][cur] = a;
	closures[my_id].data[1][cur] = b;
	closures[my_id].pc[cur] = __builtin_return_address(0);
	closures[my_id].cur_seq = (closures[my_id].cur_seq) + SHIFT + cur;
...
static uint32_t **rcas_ann;

void init_rcas_ann () 
{
	rcas_ann = (uint32_t **) malloc ((MAX_THREADS+1)*sizeof(uint32_t *));
	for (int i=0; i<MAX_THREADS+1; i++) {
		rcas_ann[i] = (uint32_t *) calloc ((MAX_THREADS + PADDING), sizeof(uint32_t));
	}
}
...
static thread_local bool rcasPerformedPeristencyInstructions = false;		
#endif
...
bool rcas_cas(RCas<PT> volatile * loc, PT exp_ptr, PT new_ptr, uint32_t tID, uint32_t opID) {
	...
	if(old_tID < MAX_THREADS && rcas_ann[tID][old_tID] < old_opID) // the read of rcas_ann[tID][old_tID] can be ld rather than ld_acq
	{
		...
		MANUAL(PWB_LOW(&rcas_ann[tID][old_tID]));
													#if defined(PROFILING) && defined(MANUAL_FLUSH)
													rcasPerformedPeristencyInstructions = true;
													#endif
	}
	RCas<PT> new_v;
