...
def plot_line_x_thread(title, subdir, name, ds_list, th_list, flush, inserts_percentage, deletes_percentage, max_work, key_range, word):
    ...
    print(title, subdir, name, ds_list, inserts_percentage,
          deletes_percentage, key_range, word)
    suffix_name = "[" + str("{0:.2f}").format(inserts_percentage) + "." + str(
        "{0:.2f}").format(deletes_percentage) + "." + str(key_range) + "]"
    ...
    for ds in ds_list:
        ...
        if word == '_num_pwbs':
            ...
            if "-Low" in ds:
                if 'Tracking' in ds:
                    ax.bar(th_list+offset, series[ds],
                           width, color='lightsalmon')
                else:
                    ax.bar(th_list+offset, series[ds],
                           width, color='lightskyblue')
                low_ds = ds
            elif "-Medium" in ds:
                if 'Tracking' in ds:
                    ax.bar(
                        th_list+offset, series[ds], width, bottom=series[low_ds], color='tomato')
                else:
                    ax.bar(
                        th_list+offset, series[ds], width, bottom=series[low_ds], color='deepskyblue')
                med_ds = ds
            elif "-High" in ds:
                if 'Tracking' in ds:
                    ax.bar(th_list+offset, series[ds], width, bottom=np.add(
                        np.array(series[low_ds]), np.array(series[med_ds])), color='r')
                else:
                    ax.bar(th_list+offset, series[ds], width, bottom=np.add(
                        np.array(series[low_ds]), np.array(series[med_ds])), color='b')
        else:
            # Copy data
            data_path = '../../out/{}.csv'.format(ds)
            if not os.path.isfile(data_path):
                with open(data_path, 'w', newline='') as f:
                    wr = csv.writer(f)
                    wr.writerow(['target', 'threads', 'duration', 'relaxed',
                                'key range', 'insert', 'delete', 'read', 'throughput'])
            with open(data_path, 'a', newline='') as f:
                wr = csv.writer(f)
                duration = 10
                relaxed = 'None'
                for i, thread in enumerate(threads):
                    throughput = series[ds][i] * 1000000
                    read_percentage = round(
                        1-inserts_percentage-deletes_percentage, 3)
                    wr.writerow([ds, thread, duration, relaxed,
                                 key_range, inserts_percentage, deletes_percentage, read_percentage, throughput])
    ...
    if low_ds == '':
        ...
        ax.legend(loc="best", labels=['Tracking [Low]', 'Tracking [Medium]',
                  'Capsules-Opt [Low]', 'Capsules-Opt [Medium]', 'Capsules-Opt [High]'])
...
def plot_preprocess1():
    ...
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '',
                           'Figure2a', algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '',
                           'Figure3a', algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '')
...
def plot_preprocess2():
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '',
                           'Figure2c', algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '',
                           'Figure3c', algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '')


def plot_preprocess3():
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '',
                           'Figure2f', algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '',
                           'Figure3f', algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '')


def plot_preprocess4():
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '',
                           'Figure4a', algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '',
                           'Figure4b', algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '')


def plot_preprocess5():
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '',
                           'Figure4c', algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '',
                           'Figure4d', algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '')


def plot_preprocess6():
    for key_range in key_ranges:
        plot_line_x_thread('Read Intensive - KeyRange:' + str(key_range), '', 'Figure2e',
                           prof_algorithms, threads, 'manual', 0.15, 0.15, 0, key_range, '_num_pwbs')
        plot_line_x_thread('Update Intensive - KeyRange:' + str(key_range), '', 'Figure3e',
                           prof_algorithms, threads, 'manual', 0.35, 0.35, 0, key_range, '_num_pwbs')
