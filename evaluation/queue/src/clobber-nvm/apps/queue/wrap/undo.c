...
POBJ_LAYOUT_ROOT(QUEUE, queue);
POBJ_LAYOUT_TOID(QUEUE, BinaryData);
POBJ_LAYOUT_TOID(QUEUE, int);
POBJ_LAYOUT_END(QUEUE);

static PMEMobjpool *pop = NULL;
static queue *popRoot = NULL; // support for only one list
int pertx_counter = 2;
...
{
    init_admin_pop();
    pop = pmemobj_open(PMemPath, POBJ_LAYOUT_NAME(QUEUE));
    if (pop == NULL)
    {
        pop = pmemobj_create(PMemPath, POBJ_LAYOUT_NAME(QUEUE), PMemSize, 0666);
    }
    else
    { // recover existing data structure
        PMEMoid root = pmemobj_root(pop, sizeof(queue));
        popRoot = D_RW((TOID(queue))root);
    }
    assert(pop != NULL);
...
{
    if (popRoot == NULL && sizeof(queue) == size)
    {
        debug("%s\n", "allocating root");
        PMEMoid root = pmemobj_root(pop, sizeof(queue));
        debug("%s: (0x%" PRIx64 ", 0x%" PRIx64 ")\n", "root", root.pool_uuid_lo, root.off);
        queue *rootPtr = D_RW((TOID(queue))root);
...
void PersistentQueueCreate(queue **q)
{
    if (popRoot == NULL)
    { // create
        queueCreate(q);
...
void PersistentqueueDestroy(queue **q)
{
}
