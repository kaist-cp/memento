...
void *worker(void *arg) {
    ...
    if (ctx->phase == 'L') {
	    ...
	    //printf("key = %s, value = %s \n", ops->key, value);
            doInsert(ctx->list, ops->key, strlen(ops->key), value, strlen(value));
            TraceOp *t = ops;
            ops = ops->next;
            free(t);
    ...
    while (ops != NULL) {
        switch(ops->opCode) {
            ...
            case Update:
                ...
                break;
            case Read:
                //doRead(ctx->list, ops->key, buffer);
                break;
        }
        TraceOp *t = ops;
        ops = ops->next;
...
uint64_t run(char workload, char phase, SkiplistPtrType list, char *tracePath,
    ...
    for (int i = 0; i < threadCount; i++) {
        ...
        SkiplistPtrAssign(context->list, list);
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, ":f:t:d:w:rh")) != -1) {
        switch(opt) {
            ...
            case 'h':
            default:
                fprintf(stdout, "Benchmark tool for the Skiplist data structure.\n");
    ...
    uint64_t exTmL = run(workload, 'L', list, tracePath, threadCount,
            valueSize, NULL);

    size_t totalOps;
    uint64_t exTmR = run(workload, 'R', list, tracePath, threadCount,
            valueSize, &totalOps);
